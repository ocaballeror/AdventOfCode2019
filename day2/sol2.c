...
int main(){
    ...
    long* registers = (long*)malloc(input->regcount * sizeof(long));
	t_memory* memory = init_t_memory(registers, input->regcount);
    ...
    for(noun=0; noun<=99 && !found; noun++){
        for(verb=0; verb<=99 && !found; verb++) {
            memcpy(memory->registers, input->registers, input->regcount * sizeof(long));
			registers[1] = noun;
			registers[2] = verb;
            start(memory);
